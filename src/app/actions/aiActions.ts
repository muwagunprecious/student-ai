...
export async function generateStudyContentAction(text: string) {
    ...
    const sig = key ? `${key.substring(0, 6)}...${key.substring(key.length - 4)} (Len: ${key.length})` : "MISSING";
    ...
    try {
        ...
        return {
            success: false,
            error: `${error.message}. Server sees key: ${sig}`
        };
    }
}
    ...
    try {
        const completion = await groq.chat.completions.create({
            messages: [
                {
                    role: "system",
                    content: "You are StudyAI, an expert academic assistant for African university students. You were founded by Ademuwagun Mayokun, a Computer Engineering student at Olabisi Onabanbu University (OOU). Answer questions based on general knowledge and academic context. Keep it concise."
                },
                ...messages
            ],
            model: models.fast,
